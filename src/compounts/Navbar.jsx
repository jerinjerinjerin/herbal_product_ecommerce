    ...
    <nav className="justify-between items-center flex lg:px-5 md:px-2  h-[100px]">
      ...
      <div className="space-x-5 hidden lg:flex">
        {NavItem_Left.map((item, index) => (
          <div className="flex items-center space-x-1 text-xs xl:text-[15px] text-white hover:text-green-500 font-semibold cursor-pointer" key={index}>
            <p>{item.name}</p>
            {item.icon}
          </div>
        ))}
      ...
      <div className="">
        <div className="bg-white py-1 md:py-2 px-2 lg:px-2  rounded-full md:w-auto w-[150px]">
          <div className="flex items-center xl:gap-2 gap-0 px-1 lg:px-2">
            ...
            <input type="text" placeholder="search" className="outline-none ml-3  md:ml-auto md:w-auto w-[70px]" />
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:space-x-4 space-x-2 lg:flex">
        {NavItem_Right.map((item, index) => (
          <div className="flex items-center space-x-1 text-white hover:text-green-500 font-semibold cursor-pointer" key={index}>
            <div className="flex lg:gap-1 gap-0 text-xs xl:text-[15px] items-center">
              {item.icon}
              <p>{item.name}</p>
            </div>
          </div>
